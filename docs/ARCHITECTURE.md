...
* **موتور استریم مستقیم (Direct Stream Engine)**: داده‌های خام خروجی دیتابیس یا فایل‌ها را دریافت، با فریمینگ رمزنگاری در حالت سکون (BPAE) محافظت کرده و مستقیماً به StorageProvider تحویل می‌دهد.
* **موتور restic (پیاده‌سازی‌شده در Future Phase A)**: به عنوان یک `BackupEngine` مستقل، وظیفه استریم امن داده‌ها به مخزن اختصاصی هر منبع با بهره‌گیری از پروتکل ناظر `Gated EOF`، ایجاد اسنپ‌شات‌های کلاینت‌ساید با برچسب‌های استاندارد، ثبت خروجی در قالب آرتیفکت‌های چندریختی (`restic_snapshot`)، اعتبارسنجی سطح ۱ (Post-Backup Verification)، و استریم دانلود در لحظه بدون نیاز به هدر Content-Length را بر عهده دارد.
...
## ۸. ارکستراسیون نگهداری مخزن و هماهنگ‌کننده درون‌برنامه‌ای (Repository Maintenance Orchestration)

برای مدیریت چرخه حیات مخازن Restic، هماهنگی عملیات هم‌روند و پیشگیری از قفل‌شدگی یا تخریب داده‌ها، معماری نگهداری مخزن (Step A.5) پیاده‌سازی شده است:

### ۱. صف پایدار عملیات نگهداری (`repository_maintenance_jobs` و `repository_maintenance_runs`):
* تمام عملیات نگهداری سنگین شامل `restic_forget` (حذف اسنپ‌شات منقضی)، `restic_prune` (هرس فیزیکی پکرها) و `restic_deep_check` (بررسی عمیق یکپارچگی داده‌ها) در یک صف پایدار در دیتابیس ثبت می‌شوند.
* کارگر اختصاصی `RepositoryMaintenanceWorker` جاب‌ها را به صورت ایمن با `FOR UPDATE SKIP LOCKED` دریافت و پردازش می‌کند.

### ۲. هماهنگ‌کننده انحصاری درون‌برنامه‌ای (`RepositoryOperationCoordinator`):
* عملیات هرس (`prune`) و بررسی عمیق (`deep check`) نیازمند قفل انحصاری مخزن (`TryAcquireExclusive`) هستند.
* در صورتی که جاب پشتیبان‌گیری فعالی روی مخزن در حال اجرا باشد، عملیات نگهداری اولویت را به بکاپ واگذار کرده و پس از اتمام آن اجرا می‌شود (`Priority Yielding`).

### ۳. سیاست چرخش قطعی بررسی عمیق (Deterministic Deep Check Rotation):
* جهت کاهش فشار پردازشی و I/O بر روی سرور، عملیات Deep Check بر روی زیرمجموعه‌های چرخشی داده‌ها ($1/N$) انجام می‌شود.
* زمان‌بند نگهداری از فرمول قطعی $\text{next} = (\text{last\_successful} \pmod N) + 1$ استفاده می‌کند. اندیس صرفاً در صورت موفقیت قطعی جاب به جلو حرکت می‌کند.

### ۴. خط‌مشی ممنوعیت مطلق آنلاک خودکار (`NO AUTOMATIC RESTIC UNLOCK`):
* بر اساس تصمیم قطعی معماری (ADR-035)، اجرای خودکار دستور `restic unlock` در سراسر پلتفرم **اکیداً ممنوع** است. شکستن خودکار قفل‌ها می‌تواند منجر به فساد فاجعه‌بار مخزن در زمان اجرای عملیات دیگر شود. مدیریت قفل‌های راکد منحصراً بر عهده اپراتور انسانی است.

### ۵. تثبیت هاست‌نیم کانتینر (`Container Hostname Pinning`):
* در محیط‌های داکر و کانتینری، مقدار `hostname: backup-platform-node-1` به صورت ثابت تعیین شده است تا متادیتای قفل‌های Restic در برابر ریستارت کانتینرها پایدار و قابل تشخیص باقی بماند.

---

## ۹. بستر و مشخصات استقرار (Deployment Concept)
...
## ۱۰. مقیاس‌پذیری و توسعه‌پذیری آینده (Future Scalability)
