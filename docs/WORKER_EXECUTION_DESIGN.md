...
| **۷. خطای پارسر یا کرش فرآیند در Gated EOF** | دریافت خطای پارسر JSON (مانند خط نامعتبر یا طول بیش از ۶۴KB) یا خروج زودهنگام Restic. | لغو فوری کانتکست تولیدکننده، ارسال سیگنال Kill به فرآیند، بستن امن stdin، بازخوانی کامل فرزند (Reap)، و ثبت خطای سیستمی بدون ثبت متادیتای ناقص. | Run: `failed`<br>Job: `failed` یا `pending` |
| **۸. شکست اعتبارسنجی سطح ۱ (Level-1 Verification)** | عدم تطابق شناسه اسنپ‌شات یا فقدان سایز منطقی (`LogicalSizeBytes <= 0`). | تفکیک خطای سرویس موقت از خطای اصالت؛ در صورت عدم وجود متادیتا یا عدم دسترسی موقت، وضعیت حفظ شده و خطا به عنوان خطای زیرساخت بازگردانده می‌شود. | Run: حفظ وضعیت قبلی یا `failed`<br>Artifact: بدون تغییر به `failed` در خطاهای زیرساختی |
...
| **Engine & Storage** | تفکیک انتزاع `BackupEngine` (شامل Direct Stream با رمزنگاری BPAE و Restic با ناظر استریم Gated EOF) از `StorageProvider` و `RepositoryTarget` | انعطاف‌پذیری در پشتیبانی از انواع بکاپ و مقاصد ذخیره‌سازی محلی و S3، استریم امن، و حذف خطرات بن‌بست I/O با پروتکل Gated EOF. |
...
## ۱۴. بخش چهاردهم: ارکستراسیون نگهداری مخازن، اولویت‌بندی و چرخه کارگر نگهداری (Repository Maintenance Worker & Orchestration - Step A.5)

با اضافه شدن موتور پشتیبان‌گیری Restic و مخازن ایزوله، نیازمندی‌های نگهداری دوره‌ای (شامل حذف اسنپ‌شات‌های منقضی، فشرده‌سازی و پاک‌سازی بلاک‌های بدون استفاده، و بررسی عمیق یکپارچگی داده‌ها) در گام A.5 پیاده‌سازی شدند:

### ۱. تفکیک صف و چرخه حیات کارگر نگهداری (`RepositoryMaintenanceWorker`):
* **صف پایدار مستقل در دیتابیس**: جدول‌های `repository_maintenance_jobs` و `repository_maintenance_runs` به صورت کاملاً مستقل از صف بکاپ‌ها طراحی شده‌اند تا از تداخل پردازش و بروز Starvation در بکاپ‌های حیاتی جلوگیری شود.
* **تصاحب اتمیک با `FOR UPDATE SKIP LOCKED`**: متد `ClaimNextMaintenanceJob` وظایف در انتظار (`pending`) را به صورت اتمیک تصاحب کرده و با ایجاد رکورد ران در وضعیت `running` و تنظیم `lease_until`، مالکیت فرآیند را به کارگر تحویل می‌دهد.
* **مکانیزم Lease و Heartbeat**: در حین اجرای عملیات نگهداری طولانی‌مدت (نظیر Prune یا Check)، یک Goroutine ناظر در فواصل ۱۰ ثانیه‌ای (`HeartbeatMaintenanceRun`) مهلت Lease را ۳۰ ثانیه جلو می‌برد.

### ۲. هماهنگ‌کننده درون‌برنامه‌ای (`RepositoryOperationCoordinator`) و تسلیم اولویت (`Priority Yielding`):
* کلیه تعاملات با مخازن Restic بر پایه شناسه یکتای مخزن (`repository_id`) توسط `RepositoryOperationCoordinator` کنترل می‌شوند:
  * **عملیات بکاپ و اعتبارسنجی سطح ۱ (Backup & Level-1 Verification)**: با قفل اشتراکی (`Shared Lock / RLock`) اجرا می‌شوند تا در صورت وجود چندین تارگت روی مخزن، بدون بلاک شدن هم‌روند اجرا گردند.
  * **عملیات نگهداری (Forget, Prune, Deep Check)**: نیازمند قفل انحصاری (`Exclusive Lock / Lock`) روی مخزن هستند.
* **سیاست تسلیم اولویت به بکاپ‌ها (Immediate Priority Yielding)**: عملیات بکاپ همواره بالاترین اولویت را در سیستم دارند. کارگر نگهداری به جای اسپین یا معطل ماندن، متد `TryAcquireExclusive(repoID)` را فراخوانی می‌کند؛ در صورتی که قفل اخذ نشود (`acquired == false`)، کارگر فوراً و بدون درنگ جاب را تسلیم کرده (`Yield`)، آن را با خطای گذرا برای تلاش مجدد با تأخیر تصاعدی (Exponential Backoff) زمان‌بندی می‌کند تا گوروتین کارگر بلافاصله آزاد شود.

### ۳. فرآیند دومرحله‌ای سیاست نگهداری: دستور `forget`، اعتبارسنجی عدم حضور و نهایی‌سازی اتمیک دیتابیس:
* **حذف منطقی تفکیک‌شده**: پردازنده Retention در صورت انقضای یک اسنپ‌شات Restic، به جای حذف ناگهانی داده‌ها، یک جاب `restic_forget` با پارامترهای اسنپ‌شات (`snapshot_id`, `artifact_id`) در صف نگهداری ثبت می‌کند (`MaintenanceEnqueuer`).
* **اعتبارسنجی ۱۰ شرط مرزی (Invariants)**: پیش از هرگونه اقدام، کارگر آرتیفکت را واکشی کرده و ۱۰ شرط قطعی (تطابق OrganizationID، ArtifactID، وضعیت عدم حذف، تطابق RepositoryID، SnapshotID با فرمت ۶۴ هگز کوچک، تطابق ResourceID و StorageTargetID) را اعتبارسنجی می‌کند. هرگونه مغایرت منجر به شکست قطعی و دائمی (`terminal failure`) جاب بدون اجرای دستور forget یا tombstone می‌گردد.
* **ترتیب قطعی و پایداری در برابر کرش**:
  1. کارگر نگهداری با در اختیار داشتن قفل انحصاری، در صورتی که فاز جاب قبلاً `forget_executed` ثبت نشده باشد، دستور `restic forget <snapshot_id>` را بدون فلگ پرهزینه `--prune` اجرا می‌کند.
  2. پس از اتمام موفقیت‌آمیز دستور، فاز جاب به صورت پایدار به `forget_executed` به‌روزرسانی می‌شود (`UpdateJobPhase`).
  3. کارگر با فراخوانی متد `VerifySnapshotAbsent` بررسی می‌کند که اسنپ‌شات قطعی از متادیتای مخزن حذف شده باشد (عدم تبدیل خطاهای شبکه یا احراز هویت به غیاب اسنپ‌شات).
  4. نهایی‌سازی اتمیک در دیتابیس (`FinalizeSuccessfulResticForget`): در یک تراکنش واحد دیتابیس، رکورد آرتیفکت با `FOR UPDATE` قفل و اعتبارسنجی می‌شود، رکورد آرتیفکت Tombstone (`is_deleted = true`, `deleted_at = NOW()`) علامت‌گذاری شده، یک جاب `restic_prune` تجمیعی با سیاست Debounce صف‌بندی می‌شود (`ON CONFLICT DO NOTHING`)، ران تکمیل و جاب نگهداری به عنوان `completed` علامت‌گذاری می‌گردد.
  5. ثبت لاگ حسابرسی `retention.cleanup` با شناسه آرتیفکت، متادیتای غیرمحرمانه و شناسه کاربر خنثی (`user_id = nil`).

### ۴. اعتبارسنجی سطح ۲ (Deep Check) با زیرمجموعه‌های پیکربندی‌پذیر و زمان‌بند مبتنی بر Keyset Pagination:
* زمان‌بند دوره‌ای `MaintenanceScheduler` با استفاده از پیمایش مبتنی بر مکان‌نما (`Keyset Pagination` بر روی ایندکس `idx_backup_repositories_active_cursor`) مخازن فعال را اسکن می‌کند.
* وضعیت استحقاق (`due`) بر مبنای عدم وجود جاب فعال، سن مخزن و گذشت بازه تنظیم‌شده (`MAINTENANCE_DEEP_CHECK_INTERVAL` با پیش‌فرض ۲۴ ساعت) از آخرین بررسی موفق ارزیابی می‌شود.
* پارامتر کل زیرمجموعه‌ها $N$ از طریق متغیر محیطی `MAINTENANCE_DEEP_CHECK_SUBSETS` (محدوده معتبر $1 \le N \le 100$ با پیش‌فرض $N = 4$) تنظیم می‌گردد. چرخش زیرمجموعه‌ها با فرمول قطعی زیر صورت می‌پذیرد:
  $$\text{next\_index} = (\text{last\_successful\_index} \pmod N) + 1$$
* پیشروی اندیس زیرمجموعه صرفاً در صورت موفقیت کامل هر دو رکورد جاب و ران (`status = 'completed'`) ثبت می‌شود.
* در زمان بوت و استارت‌آپ سرویس، متد `RecoverInterruptedMaintenanceRuns` تمامی ران‌های قطع‌شده احتمالی ناشی از کرش را به صورت امن بازیابی و جاب‌ها را با تأخیر تصاعدی بازتنظیم می‌کند.

### ۵. سیاست قطعی عدم آنلاک خودکار (`NO AUTOMATIC RESTIC UNLOCK` - ADR-035):
* اجرای خودکار دستور `restic unlock` تحت هر شرایطی در سورس‌کد سیستم اکیداً ممنوع است. باقی ماندن قفل‌های سیستمی نشان‌دهنده یک فرآیند در حال اجرای واقعی یا کرش غیرمنتظره زیرساخت است و بازگشایی خودکار آن ریسک خطای مهلک فساد فیزیکی مخزن (Repository Corruption) را به همراه دارد. هرگونه رفع قفل تنها با بررسی و مداخله مستقیم اپراتور انسانی مجاز خواهد بود.

### ۶. تثبیت هاست‌نیم کانتینر (`Container Hostname Pinning`):
* متغیر `hostname: backup-platform-node-1` در فایل `compose.yaml` تثبیت شده است تا شناسه‌های قفل Restic در ریستارت‌های سرویس پایدار مانده و از تعدد هاست‌نیم‌های تصادفی در لاگ‌های قفل مخزن جلوگیری گردد.
