...
* **سیاست تسلیم اولویت به بکاپ‌ها (Priority Yielding)**: عملیات بکاپ همواره بالاترین اولویت را در سیستم دارند. کارگر نگهداری با فراخوانی `TryAcquireExclusive` ابتدا امکان اخذ فوری و بدون درنگ قفل را بررسی می‌کند؛ در صورت اشتغال مخزن توسط بکاپ فعال، عملیات نگهداری معطل نمی‌ماند و با بازگشت به صف یا ایجاد وقفه نمایی، منابع را برای فرآیندهای حیاتی پشتیبان‌گیری آزاد می‌گذارد.

### ۳. فرآیند دومرحله‌ای سیاست نگهداری: دستور `forget`، علامت‌گذاری Tombstone و Prune با تاخیر:
* **حذف منطقی تفکیک‌شده**: پردازنده Retention در صورت انقضای یک اسنپ‌شات Restic، به جای حذف ناگهانی داده‌ها، یک جاب `restic_forget` با پارامترهای اسنپ‌شات (`target_snapshot_id`) در صف نگهداری ثبت می‌کند (`MaintenanceEnqueuer`).
* **ترتیب قطعی اجرا**:
  1. کارگر نگهداری با در اختیار داشتن قفل انحصاری، دستور `restic forget <snapshot_id>` را بدون فلگ پرهزینه `--prune` اجرا می‌کند.
  2. پس از تایید موفقیت حذف اسنپ‌شات از متادیتای مخزن، رکورد آرتیفکت در دیتابیس به صورت اتمیک Tombstone (`is_deleted = true`, `deleted_at = NOW()`) علامت‌گذاری شده و لاگ ممیزی `retention.cleanup` با شناسه آرتیفکت و متادیتای اسنپ‌شات ثبت می‌گردد.
  3. کارگر سپس یک جاب `restic_prune` تجمیعی را صف‌بندی می‌کند. به واسطه ایندکس جزئی یکتا (`ON CONFLICT DO NOTHING`)، چندین درخواست مکرر Prune در قالب یک جاب واحد ادغام (Debounce) می‌شوند تا از اجرای سنگین و پرتکرار Prune روی دیسک جلوگیری شود.

### ۴. اعتبارسنجی سطح ۲ (Deep Check) با چرخش قطعی زیرمجموعه‌ها:
* زمان‌بند دوره‌ای `MaintenanceScheduler` با فرکانس مشخص جاب‌های `restic_check` را برای مخازن فعال ثبت می‌کند.
* جهت جلوگیری از اشغال دیسک و پردازنده در مخازن چند صد گیگابایتی، بررسی داده‌ها با پارامتر چرخشی `--read-data-subset=k/N` با فرمول قطعی زیر انجام می‌شود:
  $$\text{next\_index} = (\text{last\_successful\_index} \pmod N) + 1 \quad (N = 4)$$
* پیشروی اندیس زیرمجموعه صرفاً در صورت موفقیت (`completed`) در رکورد وضعیت ثبت می‌شود؛ در صورت هرگونه خطا، اندیس تغییر نکرده و در نوبت بعدی همان بخش مجدداً مورد اعتبارسنجی قرار می‌گیرد.
