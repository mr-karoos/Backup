...
| **Charts** | **عدم نیاز در F1 Foundation / Recharts کتابخانه ارجح در صورت نیاز مصوب** | در فاز پایه‌ای F1 Foundation هیچ نیازی به کتابخانه نمودار وجود ندارد و هیچ پکیجی برای آن نصب نخواهد شد. در صورتی که در صفحات مصوب آتی (F1/F2) نیاز واقعی به نمودار تعریف گردد، **Recharts** به عنوان انتخاب مصوب و ارجح در نظر گرفته شده است. از نصب کتابخانه‌های گمانه‌زنانه نمودار خودداری می‌شود. |
...
2. **استفاده از کوکی امن با ویژگی‌های کنترلی (HttpOnly, SameSite=Strict, Secure)**:
   * صفت `HttpOnly` مانع از دسترسی و خواندن مستقیم مقدار خام کوکی رفرش‌توکن توسط کدهای جاوااسکریپت می‌شود.
   * صفت `SameSite=Strict` ریسک‌های ناشی از ارسال ناخواسته درخواست‌های بین‌سایتی (Cross-Site Requests) را به شدت کاهش می‌دهد.
   * صفت `Secure` در محیط پروداکشن از ارسال توکن بر بستر ناامن HTTP ساده جلوگیری می‌کند.
   * **تصحیح دقیق ادعای امنیتی XSS**: این کنترل‌های مرورگر هرگز به معنای مصونیت کامل اپلیکیشن در برابر حملات XSS نیستند. در صورت وقوع یک حمله موفق XSS، اسکریپت مخرب همچنان می‌تواند در بستر سشن مرورگر کاربر به اجرای عملیات بپردازد و به متغیرهای مقیم در رم مرورگر (نظیر `access_token`) دسترسی یابد.
   * بنابراین، پیشگیری از XSS نیازمند رعایت الزامات چندلایه در فرانت‌اند (نظیر استفاده از مدل فراردهی خودکار React، ممانعت از تزریق ناامن HTML، اعتبارسنجی داده‌ها و اعمال هدرهای امنیتی و CSP در پروداکشن) است.
...
5. **الزامات قطعی مقابله با XSS و پاک‌سازی داده‌ها (Strict XSS Prevention Rules for F1)**:
   * هیچ کنترلی در سطح مرورگر به تنهایی اپلیکیشن را کاملاً در برابر XSS ایمن نمی‌سازد؛ فرانت‌اند در فاز F1 ملزم به رعایت انضباط کامل در رندرینگ است:
   * **ممنوعیت مطلق dangerouslySetInnerHTML**: هرگز نباید از `dangerouslySetInnerHTML` برای داده‌های برگشتی از بک‌اند یا داده‌های تحت کنترل کاربر استفاده شود، مگر آنکه قطعه کد مشخص به طور جداگانه بازبینی امنیتی شده و با ابزارهای تاییدشده Sanitization گردد.
   * **رندر متنی مقادیر خطا**: کلیه مقادیر `error_message` در BackupRun و خطاهای API صرفاً به صورت متن خالص (Text Content) رندر می‌شوند و هرگز به صورت HTML تفسیر نخواهند شد.
   * **ممنوعیت ارزیابی پویا**: عدم استفاده از `eval()` یا ساخت و اجرای پویای کدهای جاوااسکریپت.
   * **اعتبارسنجی داده‌ها و لینک‌ها**: اعتبارسنجی تمامی ورودی‌ها و عدم استفاده از URLهای نامعتبر با پروتکل‌های خطرناک (نظیر `javascript:`).
   * **هدرهای امنیتی و سیاست امنیت محتوا در پروداکشن**: استقرار هدرهای امنیتی استاندارد در لایه وب‌سرور/پروکسی و Next.js شامل:
     * `Content-Security-Policy` (CSP) متناسب با نیازمندی‌های سیستم با محدودسازی منابع مجاز اسکریپت و استایل، و اعمال `frame-ancestors 'none'` جهت جلوگیری از Clickjacking.
     * `X-Content-Type-Options: nosniff` جهت ممانعت از تفسیر اشتباه نوع فایل‌ها.
     * `Referrer-Policy: strict-origin-when-cross-origin`.
     * `Permissions-Policy` متناسب با عملکردهای پلتفرم.
