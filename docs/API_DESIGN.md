...
* **ضوابط امنیتی دانلود**:
  ...
  2. **استریم مستقیم از طریق برنامه (Application Authorization Stream)**:
     * برای آرتیفکت‌های `direct_stream`، فایل پس از رمزگشایی درون‌مسیر (در صورت رمزگذاری BPAE) با هدرهای `Content-Disposition: attachment; filename="..."`، `Content-Type: application/gzip` و `Content-Length` متناظر ارسال می‌شود.
     * برای آرتیفکت‌های `restic_snapshot`، خروجی به کمک `restic dump` به صورت بلادرنگ به لایه فشرده‌ساز gzip هدایت شده و بدون هدر `Content-Length` از طریق Chunked Transfer Encoding انتقال می‌یابد.
...
* **مکانیزم و ترتیب اجرا (Physical Deletion First & Tombstone)**:
  1. **محدودیت نوع آرتیفکت**: حذف مستقیم آرتیفکت‌های چندریختی از نوع `restic_snapshot` از طریق این Endpoint به دلیل وابستگی اسنپ‌شات‌ها در مخزن کلاینت‌ساید مسدود بوده (`400 Bad Request / ErrDirectDeletionNotSupported`) و چرخه حیات آنها از طریق سیاست‌های Retention مخزن مدیریت می‌گردد.
  2. برای آرتیفکت‌های `direct_stream`، سیستم ابتدا از طریق `StorageProvider.DeleteArtifact(...)` فایل فیزیکی را از مقصد ذخیره‌سازی حذف می‌کند.
  3. **تنها در صورت موفقیت حذف فیزیکی**، وضعیت متادیتا در دیتابیس به عنوان Tombstone به‌روزرسانی شده و فیلدهای `is_deleted = true` و `deleted_at = NOW()` مقداردهی می‌شوند.
  4. در صورت شکست در حذف فیزیکی از دیسک/استوریج، متادیتای آرتیفکت هرگز به عنوان حذف‌شده علامت نمی‌خورد تا ناهمگونی رخ ندهد.
  5. سوابق و تاریخچه `BackupJob` و `BackupRun` هرگز توسط عملیات حذف آرتیفکت پاک نمی‌شوند و جهت سوابق عملیاتی و حسابرسی حفظ می‌گردند.
  6. ثبت اجباری رویداد `backup.delete` در جدول `audit_logs`.
...
* **سطح دسترسی مورد نیاز**: `admin`, `member`.
* **محدوده اعتبارسنجی (Verification Scope in V1 & Phase A)**:
  * **برای آرتیفکت‌های Direct Stream**:
  ...
  * **برای آرتیفکت‌های Restic Snapshot (اعتبارسنجی سطح ۱ - Level-1 Post-Backup)**:
    1. **شناسه قطعی کانونی**: بررسی وجود و یکتایی بر اساس شناسه ۶۴ کاراکتری هگزادسیمال در نمایه مخزن.
    2. **برچسب‌های شش‌گانه اجباری**: اعتبارسنجی وجود دقیق تمام برچسب‌های شش‌گانه استاندارد (`platform`, `org`, `resource`, `run`, `artifact`, `target`).
    3. **وجود نام فایل داخلی**: بررسی وجود نام فایل داخلی منتظره (`internal_filename`) در درخت اسنپ‌شات.
    4. **سایز منطقی**: بررسی بزرگتر از صفر بودن سایز واقعی فایل در اسنپ‌شات و تطابق دقیق آن با سایز منطقی ذخیره‌شده در متادیتای آرتیفکت (`fileSize == expectedLogicalSize`).
    5. **نمونه‌خوانی از دامپ**: اعتبارسنجی خوانایی نمونه اولیه تا سقف ۶۴ کیلوبایت از طریق `restic dump`.
    6. **بررسی سلامت ساختار SQL**: بررسی مارکرهای معتبر دامپ SQL در خروجی نمونه در صورت بکاپ پایگاه داده.
    7. **تفکیک خطای زیرساخت از اصالت**: تمایز دقیق خطاهای ارتباطی و زیرساختی از خطاهای فساد داده و شکست اعتبارسنجی (عدم علامت‌گذاری نادرست خرابی در خطاهای موقت).
  * *توضیح*: اعتبارسنجی سلامت ساختاری آرتیفکت را تضمین می‌کند اما ادعای ضمانت ۱۰۰٪ بازیابی منطقی دیتابیس را ندارد؛ تست کامل بازیابی به فازهای آتی موکول شده است.
