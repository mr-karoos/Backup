...
* **محدوده اعتبارسنجی (Verification Scope in V1 & Phase A)**:
  ...
  * **برای آرتیفکت‌های Restic Snapshot (اعتبارسنجی سطح ۱ - Level-1 Post-Backup)**:
    1. **بررسی وجود و یکتایی اسنپ‌شات**: استعلام متادیتا بر اساس شناسه قطعی ۶۴ کاراکتری هگزادسیمال اسنپ‌شات از مخزن.
    2. **بررسی پیوند والد و تگ‌ها**: اعتبارسنجی تطابق شناسه والد (Parent Snapshot) در صورت وجود و برچسب‌های شش‌گانه استاندارد.
    3. **بررسی سایز منطقی (`LogicalSizeBytes > 0`)**: اعتبارسنجی بزرگتر از صفر بودن حجم منطقی؛ در صورت فقدان یا صفر بودن، با خطای زیرساخت عدم دسترسی (`ErrBackupServiceUnavailable`) مسدود شده و از علامت‌گذاری نادرست خرابی اصالت جلوگیری می‌شود.
