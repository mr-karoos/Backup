...
func TestMigrations_StepA4_Integration(t *testing.T) {
	...
	locatorA := fmt.Sprintf("organizations/%s/resources/%s/restic", orgA.String(), resA.String())
	_, err = connPool.Exec(ctx, `
		INSERT INTO backup_repositories (id, organization_id, resource_id, storage_target_id, credential_id, repository_locator, status, metadata, created_at, updated_at)
		VALUES ($1, $2, $3, $4, $5, $6, 'active', '{}'::jsonb, NOW(), NOW())`,
		repoA, orgA, resA, targetA, credA, locatorA)
	if err != nil {
	...
	_, err = connPool.Exec(ctx, `
		...
		VALUES ($1, $2, $3, 'manual', 'mysql_database', 'direct_stream', $4, '{"database_name":"mydb"}'::jsonb, 'completed', NOW(), NOW())`,
		jobA, orgA, resA, targetA)
	if err != nil {
	...
	_, err = connPool.Exec(ctx, `
		...
		VALUES ($1, $2, $3, $4, $5, 'database_dump', 'sql_gzip', 'mydb', 'mydb_dump.sql.gz', 2048, 'sha256', '0123456789abcdef0123456789abcdef0123456789abcdef0123456789abcdef', 'verified', NOW(), NOW())`,
	...
	if format != "sql_gzip" || storRef != "mydb_dump.sql.gz" || sizeBytes != 2048 {
	...
	_, err = connPool.Exec(ctx, `
		...
		VALUES ($1, $2, $3, $4, $5, 'database_dump', 'restic_snapshot', 'mydb', NULL, NULL, NULL, NULL, $6, $7, $8, 'verified', NOW(), NOW())`,
	...
	_, err = connPool.Exec(ctx, `
		INSERT INTO backup_artifacts (id, organization_id, run_id, resource_id, storage_target_id, artifact_type, format, target_name, storage_reference, size_bytes, checksum_algorithm, checksum_hash, repository_id, snapshot_id, logical_size_bytes, verification_status, created_at, updated_at)
		VALUES ($1, $2, $3, $4, $5, 'database_dump', 'restic_snapshot', 'mydb', 'forbidden_ref.tar', NULL, NULL, NULL, $6, $7, $8, 'verified', NOW(), NOW())`,
	...
	_, err = connPool.Exec(ctx, `
		INSERT INTO backup_artifacts (id, organization_id, run_id, resource_id, storage_target_id, artifact_type, format, target_name, storage_reference, size_bytes, checksum_algorithm, checksum_hash, repository_id, snapshot_id, logical_size_bytes, verification_status, created_at, updated_at)
		VALUES ($1, $2, $3, $4, $5, 'database_dump', 'restic_snapshot', 'mydb', NULL, 100, NULL, NULL, $6, $7, $8, 'verified', NOW(), NOW())`,
	...
	_, err = connPool.Exec(ctx, `
		INSERT INTO backup_artifacts (id, organization_id, run_id, resource_id, storage_target_id, artifact_type, format, target_name, storage_reference, size_bytes, checksum_algorithm, checksum_hash, repository_id, snapshot_id, logical_size_bytes, verification_status, created_at, updated_at)
		VALUES ($1, $2, $3, $4, $5, 'database_dump', 'restic_snapshot', 'mydb', NULL, NULL, NULL, NULL, NULL, $6, $7, 'verified', NOW(), NOW())`,
	...
	_, err = connPool.Exec(ctx, `
		INSERT INTO backup_artifacts (id, organization_id, run_id, resource_id, storage_target_id, artifact_type, format, target_name, storage_reference, size_bytes, checksum_algorithm, checksum_hash, repository_id, snapshot_id, logical_size_bytes, verification_status, created_at, updated_at)
		VALUES ($1, $2, $3, $4, $5, 'database_dump', 'restic_snapshot', 'mydb', NULL, NULL, NULL, NULL, $6, NULL, $7, 'verified', NOW(), NOW())`,
	...
	_, err = connPool.Exec(ctx, `
		INSERT INTO backup_artifacts (id, organization_id, run_id, resource_id, storage_target_id, artifact_type, format, target_name, storage_reference, size_bytes, checksum_algorithm, checksum_hash, repository_id, snapshot_id, logical_size_bytes, verification_status, created_at, updated_at)
		VALUES ($1, $2, $3, $4, $5, 'database_dump', 'restic_snapshot', 'mydb', NULL, NULL, NULL, NULL, $6, 'not-a-valid-hex-id!', $7, 'verified', NOW(), NOW())`,
	...
	_, err = connPool.Exec(ctx, `
		INSERT INTO backup_artifacts (id, organization_id, run_id, resource_id, storage_target_id, artifact_type, format, target_name, storage_reference, size_bytes, checksum_algorithm, checksum_hash, verification_status, created_at, updated_at)
		VALUES ($1, $2, $3, $4, $5, 'database_dump', 'sql_gzip', 'mydb', '', 2048, 'sha256', '0123456789abcdef0123456789abcdef0123456789abcdef0123456789abcdef', 'verified', NOW(), NOW())`,
	...
	_, err = connPool.Exec(ctx, `
		...
		VALUES ($1, $2, $3, $4, $5, 'database_dump', 'sql_gzip', 'mydb', 'ref.gz', 2048, 'sha256', '0123456789abcdef0123456789abcdef0123456789abcdef0123456789abcdef', $6, 'verified', NOW(), NOW())`,
	...
	_, err = connPool.Exec(ctx, `
		INSERT INTO backup_artifacts (id, organization_id, run_id, resource_id, storage_target_id, artifact_type, format, target_name, storage_reference, size_bytes, checksum_algorithm, checksum_hash, repository_id, snapshot_id, logical_size_bytes, verification_status, created_at, updated_at)
		VALUES ($1, $2, $3, $4, $5, 'database_dump', 'restic_snapshot', 'mydb_dup', NULL, NULL, NULL, NULL, $6, $7, $8, 'verified', NOW(), NOW())`,
	...
	if !strings.Contains(downErr.Error(), "cannot rollback migration 000009") {
	...
	// Force version back to 9 if marked dirty from the deliberate fail-closed abort
	_ = m.Force(9)
